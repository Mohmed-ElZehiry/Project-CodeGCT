// src/features/user/services/uploads/uploadStepsClientService.ts
...
export async function getStepsClient(
  uploadId: string,
  ...
  return { success: true, data: (data || []).map(mapStepRowToUploadStep) };
}

// 🟢 إضافة خطوة جديدة
export async function addStepClient(
  uploadId: string,
  step: UploadStepType,
  details?: any,
  actor?: string,
  ...
  if (!data) return { success: false, error: "No data returned after insert" };

  return { success: true, data: mapStepRowToUploadStep(data) };
}

// 🟢 تحويل Row من DB إلى UploadStep
function mapStepRowToUploadStep(step: any): UploadStep {
  return {
    id: step.id,
